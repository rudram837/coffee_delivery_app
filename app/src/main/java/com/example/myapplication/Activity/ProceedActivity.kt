...
class ProceedActivity : BaseActivity(), PaymentResultListener {
        ...
        if (currentUserId != null) {
            ...
            databaseReference.addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onCancelled(databaseError: DatabaseError) {
                    Toast.makeText(
                        this@ProceedActivity,
                        "Failed to load price: ${databaseError.message}",
                        Toast.LENGTH_SHORT
                    ).show()
                }
            })
    ...
    private fun placeOrder() {
        ...
        if (currentUserId != null) {
            ...
            if (name.isNotEmpty() && address.isNotEmpty() && phone.isNotEmpty() && totalAmount.isNotEmpty() &&
                    ...
                    if (orderedItems.isNotEmpty()) {
                        ...
                        orderRef.setValue(order).addOnCompleteListener { task ->
                            if (task.isSuccessful) {
                                ...
                                Toast.makeText(this, "Order placement failed", Toast.LENGTH_SHORT)
                                    .show()
                            }
                        }
                    } else {
    ...
    private fun getCartItems(userId: String, callback: (List<CartItem>) -> Unit) {
        FirebaseDatabase.getInstance().getReference("users")
            ...
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    val cartItems =
                        dataSnapshot.children.mapNotNull { it.getValue(CartItem::class.java) }
                    callback(cartItems)
                }
    ...
    private fun startRazorpayPayment(amount: Double) {
        ...
        checkout.setKeyID("rzp_test_U1DIfJyKCywMJS")
        val currentUser = FirebaseAuth.getInstance().currentUser
        val email =
            currentUser?.email ?: "rudramaurya313@gmail.com"  // Fetch user email, default if null
        val phone = currentUser?.phoneNumber ?: "8369783733"  // Fetch user phone, default if null
                ...
                getCartItems(currentUserId) { orderedItems ->
                    if (orderedItems.isNotEmpty()) {
                        ...
                        if (orderId != null) {
                            ...
                            val orderReference =
                                FirebaseDatabase.getInstance().getReference("orderCompleted")
                                    .child(currentUserId)
                                    .child(orderId)
                            ...
                            orderReference.setValue(paymentDetails)
                                ...
                                .addOnFailureListener { e ->
                                    Toast.makeText(
                                        this,
                                        "Failed to update order: ${e.message}",
                                        Toast.LENGTH_SHORT
                                    ).show()
                                }
                        } else {
                            Toast.makeText(this, "Failed to generate order ID", Toast.LENGTH_SHORT)
                                .show()
                        }
                    } else {
    ...
    private fun removeCartItems(userId: String) {
        FirebaseDatabase.getInstance().getReference("users")
            ...
            .addOnFailureListener { e ->
                Toast.makeText(this, "Failed to clear cart: ${e.message}", Toast.LENGTH_SHORT)
                    .show()
            }
    }
    ...
    override fun onPaymentError(code: Int, response: String?) {
        ...
        if (currentUserId != null) {
            ...
            FirebaseDatabase.getInstance().getReference("paymentError")
                ...
                .addOnFailureListener { e ->
                    Toast.makeText(this, "Failed to log error: ${e.message}", Toast.LENGTH_SHORT)
                        .show()
                }
        }
        Toast.makeText(this, "Payment Failed. Code: $code, Response: $response", Toast.LENGTH_LONG)
            .show()
    }
